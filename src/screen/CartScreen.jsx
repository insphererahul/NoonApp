...
const CartScreen = () => {
  const { cartItems, updateCartItem, deleteCartItem, totalPrice } = useContext(CartContext);

  const handleDeleteItem = async (id) => {
    await deleteCartItem(id);
  };

  const updateItemQuantity = async (id, type) => {
  ...
  return (
    <LinearGradient colors={["#FDF0F3", "#FFFBFC"]} style={styles.container}>
      ...
      <FlatList
        data={cartItems}
        renderItem={({ item }) => (
          <CartCard item={item} handleDelete={handleDeleteItem} updateItemQuantity={updateItemQuantity} />
        )}
